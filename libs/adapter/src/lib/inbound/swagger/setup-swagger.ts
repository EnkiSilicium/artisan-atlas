...
function registerBundledSwaggerAssets(app: INestApplication) {
  ...
  const send = (res: any, contentType: string, fileName: string) => {
    ...
    if (!existsSync(p)) {
      ...
      return typeof res.send === 'function' ? res.send('Not found') : res.end('Not found');
    }
    const buf = readFileSync(p);
    // Fastify style
    if (typeof res.type === 'function' && typeof res.send === 'function') return res.type(contentType).send(buf);
    // Express style
    if (typeof res.header === 'function') res.header('content-type', contentType);
  ...
  adapter.get('/swagger/swagger-ui-standalone-preset.js', (_req: any, res: any) =>
    send(res, 'application/javascript; charset=utf-8', 'swagger-ui-standalone-preset.js'));
...
export function setupSwagger(
  ...
  { title, version = '1.0.0', path = 'docs' }: { title: string; version?: string; path?: string },
) {
  const cfg = new DocumentBuilder().setTitle(title).setVersion(version).build();
  ...
  if (process.argv.includes('--emit-openapi')) {
    ...
    writeFileSync(join(outDir, `${title}.${version}.json`), JSON.stringify(doc, null, 2));
  }
  ...
  adapter.get('/docs-json', (_req: any, res: any) => {
    ...
    return typeof res.send === 'function' ? res.send(payload) : res.end(payload);
  });
